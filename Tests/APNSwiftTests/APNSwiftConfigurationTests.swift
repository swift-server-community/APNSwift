...
"""
    ...
    func testPasswordProtectedPemWithCallback() throws {
        ...
        defer { _ = try? fm.removeItem(at: root) }

        let certFile = root.appendingPathComponent("cert.pem")
        try pemCertificate.write(to: certFile, atomically: true, encoding: .utf8)

        let keyFile = root.appendingPathComponent("key.pem")
        try pemKey.write(to: keyFile, atomically: true, encoding: .utf8)

        XCTAssertNoThrow(try APNSwiftConfiguration(privateKeyPath: keyFile.absoluteString.replacingOccurrences(of: "file://", with: ""),
                                                   pemPath: certFile.absoluteString.replacingOccurrences(of: "file://", with: ""),
                                                   topic: "", environment: .sandbox, passphraseCallback: properPassword))

        let wrongPassword: NIOSSLPassphraseCallback = { $0("foobar".utf8) }
        XCTAssertThrowsError(try APNSwiftConfiguration(privateKeyPath: keyFile.absoluteString.replacingOccurrences(of: "file://", with: ""),
                                                       pemPath: certFile.absoluteString.replacingOccurrences(of: "file://", with: ""),
