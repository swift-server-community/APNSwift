...
class APNSConfigurationTests: XCTestCase {
    ...
    func configuration(environment: APNSConfiguration.Environment) throws {
        let privateKey: APNSConfiguration.APNSPrivateKey = try .loadFrom(
            string: appleECP8PrivateKey)
        ...
        case .production:
            XCTAssertEqual(
                apnsConfiguration.environment.url, URL(string: "https://api.push.apple.com"))
        case .sandbox:
            XCTAssertEqual(
                apnsConfiguration.environment.url,
                URL(string: "https://api.development.push.apple.com"))
        }

        let loadedKey: APNSConfiguration.APNSPrivateKey = try .loadFrom(string: appleECP8PrivateKey)
        XCTAssertEqual(
            loadedKey.rawRepresentation, authenticationConfig.privateKey.rawRepresentation)
