//===----------------------------------------------------------------------===//
//
// This source file is part of the SwiftAPNS open source project
//
// Copyright (c) 2020 Kyle Browning and the SwiftAPNS project authors
// Licensed under Apache License v2.0
//
// See LICENSE.txt for license information
// See CONTRIBUTORS.txt for the list of SwiftAPNS project authors
//
// SPDX-License-Identifier: Apache-2.0
//
//===----------------------------------------------------------------------===//
#include <CAPNSwiftBoringSSLShims.h>
...
int CAPNSwiftBoringSSLShims_EVP_AEAD_CTX_init(EVP_AEAD_CTX *ctx, const EVP_AEAD *aead,
                                            ...
                                            ENGINE *impl) {
    return CAPNSwiftBoringSSL_EVP_AEAD_CTX_init(ctx, aead, key, key_len, tag_len, impl);
}
...
int CAPNSwiftBoringSSLShims_EVP_AEAD_CTX_seal_scatter(
    ...
    const void *ad,
    size_t ad_len) {
    return CAPNSwiftBoringSSL_EVP_AEAD_CTX_seal_scatter(ctx, out, out_tag, out_tag_len, max_out_tag_len, nonce, nonce_len, in, in_len, extra_in, extra_in_len, ad, ad_len);
}
